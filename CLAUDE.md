...
2026年は **9/9から28℃で稼働し、以降の仕込みは全て暖房室で熟成させる**（ユーザー確認）。
...
- **`heatingDefaultTemp`（暖房室の設定温度）と `q10BaseTemp`（Q10の基準温度）を2026-09-10に分離した**。
  ...
  （現在: 暖房28℃ / Q10基準25℃）。`RoomTemps.heatingBaseTemp` に渡すのは `q10BaseTemp` の方。
- 効果（実データ・目標600℃・日）: 9/16仕込みの熟成日数 41日→33日、10/7仕込み 39日→32日。
