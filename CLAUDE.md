...
- **予定出荷（大口）入力**: 品種ごとに「出荷予定日＋kg」を登録（`localStorage`: `planning_scheduledOrders_{name}`、JSON配列）。**未来分のみ**を負の補充イベント（`{date, kg: -kg}`）として既存の `supplyEvents` パイプライン（`findStockOutDate`・`computeSupplyReceived`）に合流し、在庫切れ日を前倒し→推奨仕込み日を前倒し。過去日（≦今日）は現在庫に織り込み済みとみなし反映外。統計予測が知り得ない確定受注を人手で注入する用途
- **在庫切れリスク警告バナー**: 1回目推奨日が過去の場合、超過日数・有効在庫・消費ペース・推定在庫切れまでの日数を表示（予定出荷も反映済み）
