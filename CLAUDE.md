...
- **仕上がりプロファイル帯**：収穫窓中央で評価した味の傾向（甘味・旨味・苦味・酸味・焦げ）を横バー＋基準マーカーで表示。**基準（中央50%）＝標準みそ（`STANDARD_KOJI_HO=10`割・`STANDARD_SALT_PCT=11`%）**。バーは対数目盛（`50+25*log2(ratio)`。2倍/半分で±25%）。甘味＝bMax×穀物量、旨味＝アミノ酸率×大豆量（甘味と対称）、苦味・焦げは低いほど良い軸。※モデルのキャリブレーション正規化（K_AMY_BASE定義点＝無添加麦24.1割）は別物で不変
