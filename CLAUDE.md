...
### 暖房室の稼働開始日（前倒し稼働・2026-09-09）

通常は10月に入ったら常温のロットを暖房室へ移す運用だが、**熟成が遅れた年は前倒しで稼働させる**。
2026年は **9/9から29℃で稼働し（9/13から31℃）、以降の仕込みは全て暖房室で熟成させる**（ユーザー確認）。

- 判定は `lib/tempCalc.ts` の **`isHeatingDate(date, heatingStartDate)`** に一元化（`isOutdoorDate` はその否定）。
  「10〜5月＝暖房」に加えて、`heatingStartDate` が入っている年はその日から9/30までも暖房として扱う。
  月境界を直に書いた判定（`month >= 6 && month <= 9`）を新しく増やさないこと。
- 設定は SystemSetting の **`aging_heatingStartDate`**（`yyyy-MM-dd`・空なら従来どおり）。設定画面「熟成の条件」タブで編集。
  前倒しした月（9月など）は `HEATING_MONTHLY_FACTOR` に実績が無いので補正なし（=1.0）で積む。
- **`heatingDefaultTemp`（暖房室の設定温度）と `q10BaseTemp`（Q10の基準温度）を2026-09-10に分離した**。
  以前は1つの値が両方を兼ねており、暖房室を28℃に上げるとQ10の基準温度まで動いて常温の積算が
  約2割変わってしまうため。**暖房室の温度を変えるときに触るのは `heatingDefaultTemp` だけ**
  （現在: 暖房31℃ ※9/13から。9/9〜9/12は29℃ / Q10基準25℃）。`RoomTemps.heatingBaseTemp` に渡すのは `q10BaseTemp` の方。
- 効果（実データ・目標600℃・日）: 9/16仕込みの熟成日数 41日→31日、10/7仕込み 39日→31日。
- **暖房室を明示的に選んだ場合も月別補正係数を掛ける**（`simulateHeatingDays()`・2026-09-10）。
  以前は「暖房固定＝目標÷日次レートの固定日数」だったが、通年で暖房室を使うようになったので
  12月が遅く春が速い季節カーブを無視できなくなった。仮登録の場所も暖房期の分は `暖房XX℃` に寄せる
  （中身は元から暖房として積んでいたのに画面の場所だけ常温のままで紛らわしかった）。
- **仮登録（BrewPlan）は登録時点の完成予定日・熟成日数をDBに持っている**ので、設定を変えても
  古い日数のまま残る。仮登録リストの「熟成日数を引き直す」ボタン（`recalcPendingBrewPlans()`）で
  全件引き直す。暖房室の設定温度が変わっていれば `暖房XX℃` の場所も現在の温度に合わせる。
...
- **出荷日（`Lot.shippedAt`・2026-09-10追加）**: 出荷済にした日。完成日とは別に持つ（完成日は
  「完成」にしたときだけ書く）。出荷済のときヘッダーに「出荷日：… （完成から N日）」を出し、
  鉛筆で編集できる（`updateShippedAt`）。熟成中などへ戻すと消える。**この機能より前に出荷済に
  したロット（249件）は記録なし**——過去の日付は復元できないので手で入れる
- 完成日・実際の完成日には**熟成日数を併記**する（`完成日：2026年7月29日（熟成42日）`）
- 積算温度グラフ（`LotSimChart`）:
...
- **全桶が空になったときの「出荷済みにしますか？」（2026-09-10修正）**: 出すのは**空になった瞬間だけ**
  （`wasAllEmptyRef` の遷移で判定）。画面を開いた時点で既に全桶が空でも出さない。以前は初期値でも
  出していたため、出荷済みにした直後の `router.refresh()` で同じダイアログが復活していた。
- **完成日（`completedAt`）は「完成」にするときだけ書く**（2026-09-10修正）。出荷済・品質低下出荷・
  種みそ転用への変更で今日の日付を入れると、実際の完成日（手で直した値を含む）が消えて熟成日数が狂う。
  `changeLotStatus` は日付が渡されなければ既存値を保つ
...
#### ①' 3品種まとめての提案（CombinedBrewPlan・2026-09-04）

提案の出し方を **「品種ごと」/「3品種まとめて」** で切り替えられる（`viewMode`）。品種ごとの提案は `calcBatches` を品種別に独立して回すため、**田舎と無添加が同じ週を欲しがったときにどちらかが押し出されることを表現できず、山吹の「単独では仕込めない」も出せない**（2品種同時の制約のため）。まとめモードは品種ごとの提案を入力として、現場の組み合わせルールで週の水木2枠へ置き直す（`lib/brewCombine.ts`・純粋関数）。

現場ルール（2026-09-04 ユーザー確認）：
- **絶対** ①仕込みは水・木のみ ②山吹は木曜のみ・同じ週の水曜に田舎か無添加が要る ③セットで組むときは水＝田舎／木＝無添加 ④単発なら水木どちらでもよい（品種を問わない）
- **なるべく** ⑤水木セットが基本・単発はコスト（夏は在庫条件から自然に増える。「夏＝単発」と決め打ちしない） ⑥セット間隔2〜3週

置き直しで日がずれた回は、**完成予定日を `RecipePlan.getCompletion` で引き直し**（常温は季節で熟成日数が変わるため）、原料手配締切も逆算し直す。仮登録済みは動かさず先に枠を埋める。**未検証**：品種ごとの提案を入力に使っているため、まとめモード単体で人が組んだ2026年の並びを再現できるかはまだ確かめていない（`scripts/debug-brew-plan.mts` と同じ作りで検証スクリプトを用意するのが次の一手）。

#### ①'' 振り返り（RetrospectPanel・2026-09-08）

**予測を使わず実績だけ**で在庫推移を再現し、安全在庫ラインを割った期間・深さと「いつ・何回仕込んでおけばよかったか」を出す。起点＝月末在庫スナップショット（熟成済＋小分け）、消費＝出荷実績の日割り、補充＝実際に完成したロット。計算は `lib/retrospect.ts`（純粋関数）、データ収集は `app/planning/retrospect-action.ts`（重いのでパネルを開いたときだけ実行）。CLI版は `scripts/retrospect.mts`。

**踏んだ罠（同じ間違いをしないこと）**：
- **熟成日数に `completedAt − brewedAt` を使わない**。出荷済ロットの `completedAt` は使用開始日で、実データで95日などになる（[[project_completedat_is_usestart]]）。日数は `simulateFermentationDays` で出す
- **補充に `completedAt` を使うと在庫が1回分多く出る**。出荷済ロットではその日より前に完成して起点在庫に入っているため。月末スナップショットで毎月つなぎ直してズレを断ち切る（グラフの月初の段差＝再現と実測の差）
- **月末以外に在庫スナップショットを手動実行しない**。保存スクリプトは実行日の前月をラベルにするので、その日の在庫が前月末として保存される。2026-05-30の手動実行で作られた「2026-04」が実例（中身は5/30の値・2026-09-09に削除済み）。`save_inventory_snapshot.py` は月初1日以外では中止するようにした（補完時は `SNAPSHOT_YEAR_MONTH=YYYY-MM` を明示）。振り返り側も記録日時が月末から3日以上離れたスナップショットは使わない
...
蒸した麦・砕米を放冷機で冷やしたときの設定（ファン・ベルト）と結果（品温）の記録。
2022-09〜の `放冷機パラメータ.xlsx`（大道工場共有）から移行した（151回・設定行300件）。

- **`CoolingRun`（作業日1日＝1件）＋ `CoolingStep`（その日に設定を変えた都度の行）** の親子。
  エクセルが「日付の行の下に2回目・3回目がぶら下がる」形だったのをそのまま持っている
- **仕込みは放冷の2日後**（`COOLING_TO_BREW_DAYS`）。実績151回中150回がこの形で、例外は
  2025-04-22（翌日の2025-04-23に仕込み）1件のみ。同じ日に複数仕込んだ日は原料で絞る
  （**砕米＝山吹みそ**。実績9回すべて山吹・2026-09-11ユーザー確認。白みそは無洗米で放冷機を
  通さない。対応は `lib/cooling.ts` の `MISO_TYPES_BY_GRAIN`）
- **放冷の時点ではロットがまだ無い**（ロットは仕込み日に登録する）ので、保存時は `lotId` が
  null のことがある。ロット登録の最後に `relinkCoolingRuns()` を呼んで後追いで結んでいる
- **品温は原文を残す**。現場は「37～38」「30℃以下」「(40?)」のように幅や但し書きで書くため、
  `productTempMin` / `productTempMax` に数値化しつつ `productTempRaw` に原文を保存し、表示は
  原文優先（`lib/cooling.ts` の `parseProductTemp` / `formatProductTemp`）
- 画面は **入力＋「気温が近い日の設定」＋これまでの一覧**。ベルトは気温にほぼ連動する
  （夏27・冬70）ので、1Fの気温を入れると近い過去3回の「ベルト→品温」が出る。
  一覧の「編集」（または日付・参照の日付）で記録を入力欄に読み込み、入力欄までスクロールして**編集モード**
  （枠が琥珀色・「編集をやめて新規入力に戻る」あり）。編集中の記録は **id で持つ**ので日付欄を直すと
  その記録の日付を移せる（移した先に別の記録があれば保存を弾く・ロットは2日後で紐付け直す）。
  ロット詳細の「この記録を編集」は `/cooling?date=yyyy-MM-dd` で開く
- 取り込みは `npx tsx scripts/import-cooling.mts <xlsx> [--dry]`（`runDate` でupsert・再実行可）
- **ベルトと品温の目安（`lib/coolingModel.ts`・2026-09-14）**：入力欄の気温(1F)から
  「いつものベルト→予測品温」と「目標品温にするためのベルト」を出し、各行の品温欄の
  placeholderにその行のベルトでの予測を出す。式は **品温 ≒ a + b×気温(1F) + c×ベルト** の重回帰で、
  ページを開くたびに**直近3年**の記録から作り直す（記録が増えれば自動で更新）
  - 検証（2026-09-14・麦・ファン50・日ごとに抜いて当てる）：直近3年 100日/162件で平均誤差0.76℃・
    8割が±1.17℃以内（何も使わず平均で当てると1.96℃）。2F気温・室温より1F気温が当たる。
    2022年分は約1℃低く出るため全期間より直近の方が当たる
  - **対象は麦・ファン50のみ**。空欄のファン/ベルトは同じ日の前の行から引き継ぐ。「30℃以下」のような
    片側だけの品温は使わない。砕米は9日しかなくファンも毎回違うので予測を出さない
  - 逆算（目標品温→ベルト）は式を解くだけなので、気温・ベルトが記録の範囲外なら「目安程度」と注意を出す
  - 「いつものベルト」は各日の最初の設定を気温で回帰したもの（人の初手。誤差±3程度）
