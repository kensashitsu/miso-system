...
- **計算の根拠**: 折りたたみ内に**在庫推移グラフ**（`StockProjectionChart.tsx`・完成補充を織り込んだ日次在庫見込みAreaに手配締切/仕込み日/完成/在庫切れの縦線。熟成中ロット（悲観モードのみ）と仮登録（常時）の完成ジャンプには**桶番号ラベル**（緑点）。Q10補正ありチェーン基準）＋全回分のテキスト（①消費量推計 ②有効在庫 ③在庫切れ日〜仕込み日 ④手配締切）
